...
<head>
  ...
  <title>iFiLMS Recycle Bin</title>
      ...
      <main class="flex-1 p-6">
        ...
        <div class="flex justify-between items-center mb-6">
          <div class="flex items-center gap-3 text-lg font-semibold">
            <i data-lucide="recycle"></i> List of Files Staging for Deletion
          </div>
        ...
        <!-- Results Table -->
        <div class="border border-gray-200 dark:border-gray-700 rounded-2xl bg-white dark:bg-gray-900 shadow-sm overflow-x-auto">
          <table class="w-full text-sm">
            <thead class="bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 uppercase text-xs font-semibold">
              <tr>
                ...
                <th class="px-6 py-3 text-left">Date Deleted</th>
                <th class="px-6 py-3 text-left">Deleted By</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700 text-gray-800 dark:text-gray-100">
              @forelse ($documents as $document)
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                  ...
                  <td class="px-6 py-3">{{ $document->deleted_at ? $document->deleted_at->format('Y-m-d H:i:s') : '—' }}</td>
                  <td class="px-6 py-3">{{ $document->deletedByStaff->name ?? 'Unknown' }}</td>

                  <!-- Actions -->
                  <td class="px-6 py-3 text-center">
                    <div class="flex justify-center gap-2">
                      <a href="{{ asset('storage/' . $document->file_path) }}" target="_blank"class="px-3 py-1 rounded-lg bg-blue-500 hover:bg-blue-600 text-white transition">
                        View
                      </a>
                      <form action="{{ route('recyclebin.restore', $document->document_id) }}" method="POST" onsubmit="return confirm('Restore this document?');">
                        @csrf
                        <button type="button"onclick="openRestoreModal('{{ route('recyclebin.restore', $document->document_id) }}')"class="px-3 py-1 rounded-lg bg-green-500 hover:bg-green-600 text-white transition">
                          Restore
                        </button>
                      </form>
                      <form action="{{ route('recyclebin.destroy', $document->document_id) }}" method="POST" onsubmit="return confirm('Permanently delete this document? This cannot be undone.');">
                        @csrf
                        @method('DELETE')
                        <button type="button"onclick="openDeleteModal('{{ route('recyclebin.destroy', $document->document_id) }}')"class="px-3 py-1 rounded-lg bg-red-500 text-white hover:bg-red-600 transition">
                            Delete
                          </button>
                      </form>
                    </div>
        ...
        <div id="restoreModal"class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50">
          <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg w-96 text-center">
            <h2 class="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">
              Restore Document?
            </h2>
            <p class="text-gray-600 dark:text-gray-400 mb-6">
              Are you sure you want to restore this document from the Recycle Bin?
            </p>
            <div class="flex justify-center gap-4">
              <button id="confirmRestoreBtn"class="px-4 py-2 rounded-lg bg-green-600 text-white hover:bg-green-700 transition">
                Yes
              </button>
              <button onclick="closeRestoreModal()"class="px-4 py-2 rounded-lg bg-gray-300 dark:bg-gray-700 text-gray-900 dark:text-gray-200 hover:bg-gray-400 dark:hover:bg-gray-600 transition">
                No
              </button>
            </div>
          </div>
  ...
  <script>
    ...
    //Restore Modal Logic
    function openRestoreModal(action) {
      restoreFormAction = action;
